...
def main():
    ...
    robot = Robot(n_rays=180, spread=np.pi, speed=0.5, angvel=1.0, reactrange=3, k=0.05)
    env = Environment(robot=robot, map=my_map)
    
    slammer = GMapper(env,30,resampling_temperature=3.0,search_distance=4)
    dt = 0.15
    
    PANEL_SIZE = 600 
    
    while True:
        action, scan = slammer.step_env(dt)
        slammer.update_particles(action, scan, dt)
        
        img_true, img_pred = slammer.render(lidar_scan=scan)
